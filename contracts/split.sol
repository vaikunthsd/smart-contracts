...
contract Split {
  ...
  mapping(address => mapping(uint => uint)) public splits; //nested mapping is proposalId to valuation at beginning of voting periode
  ...
  function deposit() external payable {
    ...
    if(unsealedProposal == true && myVote != Vote.UNDEFINED && splits[msg.sender][nextProposalId - 1] == 0) {
  ...
  function withdraw(uint amount) external {
    ...
    if(unsealedProposal == false && splits[msg.sender][nextProposalId -1] != 0 && valuation > 0) {
      _valuation = splits[msg.sender][nextProposalId - 1];
    }
  ...
  function split() external {
    require(unsealedProposal == true);
    require(votes[msg.sender][nextProposalId - 1] != Vote.UNDEFINED);
    splits[msg.sender][nextProposalId - 1] = valuation;
  }
